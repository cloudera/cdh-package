...
BLACKLIST='/usr/java/jdk1.6.0_31
    /usr/lib/jvm/java-1.6.0-openjdk-1.6.0.34.x86_64
    /usr/lib/jvm/jre-1.6.0-openjdk.x86_64'

# For each path, create it and install a 'java' executable 
for JDK in ${JDKs} ${BLACKLIST} ; do
    mkdir -p ${JDK}/bin
    touch ${JDK}/bin/java
done
...
#Make sure that we do not use blacklisted versions of Java
for JDK in ${BLACKLIST} ; do
    unset JAVA_HOME
    source /bin/bigtop-detect-javahome
    if [ "${JAVA_HOME}" == ${JDK} ]; then
        echo "${JAVA_HOME} set to a black-listed version of Java ${JDK}"
        exit 1
    fi
    rm -r ${JDK}
done
