...
cp -R ${BUILD_DIR}/oozie-server/webapps ${SERVER_LIB_DIR}/tomcat-webapps

# Unpack oozie.war some place reasonable
OOZIE_WEBAPP=${SERVER_LIB_DIR}/tomcat-webapps/oozie
mkdir ${OOZIE_WEBAPP}
unzip -d ${OOZIE_WEBAPP} ${BUILD_DIR}/oozie.war
mv -f ${OOZIE_WEBAPP}/WEB-INF/lib ${SERVER_LIB_DIR}/libserver
touch ${SERVER_LIB_DIR}/tomcat-webapps/oozie.war
...
mv ${SERVER_LIB_DIR}/tomcat-webapps/oozie/WEB-INF ${HTTP_DIRECTORY}/WEB-INF
...
ln -s /var/lib/oozie/tomcat-deployment/WEB-INF ${SERVER_LIB_DIR}/tomcat-webapps/oozie/
...
timestamps='s#-[0-9]\{8\}\.[0-9]\{6\}-[0-9]\{2\}##'
for dir in ${SERVER_LIB_DIR}/libtools ${SERVER_LIB_DIR}/libserver ; do
    for old_jar in `ls $dir/avro-*.jar` ; do
        ...
        new_jar=`echo \`basename $old_jar\` | sed -e $versions | sed -e $timestamps`
