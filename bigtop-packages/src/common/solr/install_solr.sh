...
sed -i -e 's#/../solr-webapp/webapp/WEB-INF/lib/#/../webapps/solr/WEB-INF/lib/#' $PREFIX/$LIB_DIR/bin/{zkcli.sh,sentrycli.sh,snapshotscli.sh}
...
if [ -n "${SOLR_SSL_ENABLED}" ]; then
  tomcat_set_prop "solr.tomcat.ciphers=$SOLR_CIPHERS_CONFIG"
fi
